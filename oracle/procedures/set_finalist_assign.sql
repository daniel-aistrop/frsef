create or replace procedure set_assign_finalist (
...
*/
   ...
   ----------------------------------
   update zfair_projects_finalist
   set judge_count = 0
   ...
   */
   ...
   v_project_slots := max_project_slots(v_division);
   ...
   for project in (
      ...
      loop
         ...
         */
         ...
         from (
            select g.group_name
            from zfair_groups_finalist g
            left join zfair_assign_finalist_groups a
               on g.group_name = a.group_name
               and v_zfair_id = a.zfair_id
            where (split_division(g.group_name) = v_division and split_category(g.group_name) = v_categories)
               and a.zfair_id is null
            order by g.project_count asc, dbms_random.random asc 
         ) a
         where rownum = 1;

         /*The insert and updates is how the cursor adapts on the next run.*/
         begin
            insert into zfair_assign_finalist_groups (zfair_id, project_id, group_name, timeslot)
            values (v_zfair_id, v_project_id, v_group_name, null);
